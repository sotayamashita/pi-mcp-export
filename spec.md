...
| 2026-04-19 | Phase 5 で `inspect` サブコマンド / `--strict` / `--timeout` を実装。strict は **load-time** 検出のみ（`pi.on("agent_start", ...)` / `registerShortcut(...)` 等の登録時点で `LoadDiagnostics` に蓄積、CLI が exit 1）。runtime API（`ctx.ui.confirm` / `setWidget` 等）の strict 検出は次フェーズ以降 | §12.15。load-time 観測は `createExtensionApi` に optional `LoadDiagnostics` を通すだけで実現でき、非対応の実質 100% は `registerShortcut` と非対応 event 登録で捕まる。runtime API は execute 中にしか呼ばれず、load-time では検出不能なので別設計が必要                                                                                                                                                                            |
| 2026-04-19 | `--timeout` のデフォルトは **0（無効）**。spec §6.3 の「default 300」から変更                                                                                                                                                                                                                         | Codex review round 6 の指摘。300 を既定にすると既存 extension の長時間処理（`run_experiment` 等、10 分以上走るケース）が silent に falure する破壊的変更となる。明示的な opt-in を要求することで surprise なし。spec §6.3 の表記は `--timeout <seconds>` とし default は 0 に更新                                                                                                                                                   |
| 2026-04-19 | `--timeout` 適用範囲は **tool_call pre-hook + tool.execute + tool_result post-hook の dispatch 全体**                                                                                                                                                                                                 | Codex review round 2/3/7/11 の指摘累積。pre-hook が hang / execute が signal 無視 / post-hook が hang するいずれのケースでも dispatch を bounded に止める必要あり。(1) `AbortSignal.any([opts.signal, timeoutCtrl.signal])` を全段で共有 (2) `Promise.race` で実行自体も bound (3) 同期 throw も promise 化で捕捉 (4) timeout 後 post-hook も観測のため起動するが個別 100ms budget で capped (5) timeout 後 progress 更新を silence |
| 2026-04-19 | inspect の command 分類は prefix ではなく **登録経路の明示 tracking**                                                                                                                                                                                                                                 | Codex review round 4。`command_*` prefix だけで classify すると `registerTool({name: "command_status", ...})` が command と誤認される。`createExtensionApi` / `loadExtension` に optional `commandNames: Set<string>` を通し、`registerCommand` 成功後のみ set 追加、inspect はこの set を参照                                                                                                                                      |
| 2026-04-19 | `pi.on(event, ...)` で SUPPORTED/UNSUPPORTED いずれにも載らない event 名は `unknown-event` diagnostic として記録                                                                                                                                                                                      | Codex review round 2。typo（`session_strat` など）や将来追加予定イベントを silent に受け入れると strict / inspect が意味を失う。support-policy の 2 マップ（SUPPORTED_EVENTS / UNSUPPORTED_EVENTS）に存在しないものは第三カテゴリとして明示化                                                                                                                                                                                       |
| 2026-04-19 | `--strict` diagnostic は拡張パス単位で grouping して出力                                                                                                                                                                                                                                              | Codex review round 9。複数 `--extension` 構成でどの拡張が原因か不明になると対処不能。`serve` では per-extension の `LoadDiagnostics` を保持し、summarize 時に path prefix を付与                                                                                                                                                                                                                                                    |

## 11. 進捗 (Progress)

**最終更新:** 2026-04-19 / **現在位置:** Phase 5 完了、Phase 6 未着手。
...
- **`inspect` サブコマンド**: `pi-mcp-export inspect --extension <path> [--format text|json]`。拡張を stdio サーバ開かずに load し、`InspectReport`（tools / commands / events supported/unsupported/unknown / unsupportedApis）を出力。単一 `--extension` は JSON object、複数なら JSON array
- **`--strict` フラグ**: `serve` 起動時に `LoadDiagnostics` が非空なら stderr に path prefix 付きサマリを出して exit 1。診断カテゴリは unsupported-event / unknown-event / unsupported-api
- **`--timeout <seconds>` フラグ**: `serve` の per-tool-call タイムアウト。default は **0（無効）** — 既存拡張の長時間処理を破壊しない。`0.001` 未満は reject。`1ms` のような単位付き値も regex で reject
- **support-policy**: `SUPPORTED_EVENTS` (4) / `UNSUPPORTED_EVENTS` (9、reason 付き) / `UNSUPPORTED_APIS` の中央集権リスト
- **timeout の dispatch 全域適用**: `AbortSignal.any([opts.signal, timeoutCtrl.signal])` を ctx + execute に共有、`Promise.race` で pre-hook / execute / post-hook いずれの hang も bound。timeout 後も post-hook は audit のため起動（100ms budget で capped）、override は無視、progress 送信は silence、flush もスキップ
- **commands 分類の正確化**: inspect は `registerCommand` が実際に成功したものだけを command として扱う（prefix 推測を廃止）
- **検証**: 16 テストファイル / 109 ケース GREEN、`pnpm typecheck` / `pnpm lint` クリーン。Codex review 12 ラウンドで P1 2 件（fail-closed race、synchronous execute throw propagation）と P2 17 件・P3 3 件を TDD 修正。round 12 で指摘 0 件収束

### 未実装（Phase 6 以降）

- `ctx.ui.setWidget` / `ctx.ui.custom` の実描画（MCP プリミティブ追加待ち）
- runtime API の strict 検出（`ctx.ui.confirm` など execute 中にしか観測できない non-supported API）
- `--extensions-dir` / `npm:` 参照（spec §4.5）
...
### 12.15 Phase 5 の timeout 境界線は dispatch 全域 + 段階的 budget で収束した
...
| sync throw の escape         | execute() call を try の外に移動         | synchronous throw が dispatch から逃げる                   | `Promise.resolve(execute())` + try/catch                              |
| timeout 後の post-hook 欠落  | loop 全体を skip                         | 複数 audit handler のうち最初の hang で後続が見えなくなる  | timeout 後も loop 継続、override は skip、budget でキャップ           |
| timeout 後の override        | post-hook の override を適用             | `{isError: false, content: "ok"}` で timeout 隠蔽される    | `timedOut` フラグで override と read-back を両方 skip                 |
| progress の遅延配送          | 既存の 5s flush を続ける                 | timeout 発火後に更に 5s 待ち → 体感 timeout + 5s           | timeout 後は flush skip、`onUpdate` も silence                        |
...
| inspect 並列 load            | `Promise.all(extensions.map(load))`      | 拡張の startup side effect が並列で衝突                    | serve と同じく sequential for-loop                                    |

12 ラウンドで P1 2 件（timeout race / sync throw escape）、P2 17 件、P3 3 件を解消。**Phase 3-4 より長く回す必要があった理由**: timeout は "時間軸の横断的制約" であり、dispatch の全ステージ（pre-hook / execute / post-hook / progress / flush）に独立な race condition が存在する。各ステージを単独で見ると正しく見えても、組合せで穴が出る。段階的に fail scenario を提示してもらうことで、spec §6.3 の一行要件（`--timeout <sec>`）が要求する実装を 15 件以上の細かい決定に分解できた。**残る limitation**: timeout 発火後の post-hook は individual budget (100ms) で capped、非協力的な tool は abort signal を無視する限り Node の event loop 次 tick まで生存する可能性（ctx.signal 経由の pi.exec 連鎖 SIGTERM → 500ms → SIGKILL で実プロセスは殺せるが、pure JS の busy loop は kill できない）。ドキュメント化のみ。
